...
appid = "2657638375"    # 项目的 appid
token = "NHt65iYV2xQ-0Uv6VfO97BletTaOMtAn"    # 项目的 token
cluster = "volcengine_streaming_common"  # 请求的集群
audio_path = "/home/d3lab/Projects/RemoteControlDog/robot_dog_python/woz_system_backend/output.wav"  # 本地音频路径
...
class AsrWsClient:
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        results = []  # ✅ 存储所有识别片段
        from websockets.client import connect  # ✅ 确保用新版本接口
        ...
        async with connect(self.ws_url, extra_headers=header, max_size=1000000000) as ws:
            ...
            for seq, (chunk, last) in enumerate(AsrWsClient.slice_data(wav_data, segment_size), 1):
                ...
                if 'payload_msg' in parsed:
                    print("[Partial]", parsed['payload_msg'])  # ✅ 实时打印
