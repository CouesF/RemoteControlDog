...
if __name__ == '__main__':
    ...
    # === 分阶段恢复：RF_1 → RF_2 → RL_0 → RL_1 → RL_2 ===
    restore_step1 = step4[:]
    restore_step1[1] = stand_pos[1]  # RF_1 thigh 收回
    interpolate_selected_joints(step4, restore_step1, [1], 250, low_cmd, publisher, crc)

    restore_step2 = restore_step1[:]
    restore_step2[2] = stand_pos[2]  # RF_2 calf 收回
    interpolate_selected_joints(restore_step1, restore_step2, [2], 250, low_cmd, publisher, crc)

    restore_step3 = restore_step2[:]
    restore_step3[9] = stand_pos[9]  # RL_0 hip 收回
    interpolate_selected_joints(restore_step2, restore_step3, [9], 500, low_cmd, publisher, crc)

    restore_step4 = restore_step3[:]
    restore_step4[10] = stand_pos[10]  # RL_1 thigh 收回
    interpolate_selected_joints(restore_step3, restore_step4, [10], 500, low_cmd, publisher, crc)

    restore_step5 = restore_step4[:]
    restore_step5[11] = stand_pos[11]  # RL_2 calf 收回
    interpolate_selected_joints(restore_step4, restore_step5, [11], 500, low_cmd, publisher, crc)
