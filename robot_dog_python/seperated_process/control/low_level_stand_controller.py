...
def run_lowlevel_stand_hold():
    """保持底层模式并持续发送标准站立姿态指令（无限循环，由主状态机控制）"""
    lowcmd = unitree_go_msg_dds__LowCmd_()  # 使用 wrapper 初始化
    publisher = ChannelPublisher("rt/lowcmd", type(lowcmd))  # 使用 type() 注册类型
    publisher.Init()
    crc = CRC()
